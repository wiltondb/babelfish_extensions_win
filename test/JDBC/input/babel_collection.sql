...
GO
-- test catalog
select name collate "C", description from sys.fn_helpcollations() order by name;
GO

SELECT set_config('babelfishpg_tsql.sql_dialect', 'tsql', false);
GO
-- test collation list sys table
SELECT collation_name collate "C", l1_priority, l2_priority, l3_priority, l4_priority, l5_priority FROM sys.babelfish_collation_list() order by collation_name;
GO
-- clean up
drop table testing1;
GO
