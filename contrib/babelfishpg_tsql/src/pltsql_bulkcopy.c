...
BulkCopy(BulkCopyStmt *stmt, uint64 *processed)
{
	...
	PG_CATCH();
	{
		...
		elog(WARNING, "Error while executing Bulk Copy. Error occured while processing at "
			 "implicit Batch number: %d, Rows inserted in total: %lld", stmt->cur_batch_num, stmt->rows_processed);
		if (rel != NULL)
	...
	elog(DEBUG2, "Bulk Copy Progress: Successfully inserted implicit number of batches: %d, "
		 "number of rows inserted in total: %lld, "
		 "number of rows inserted in current batch: %lld",
...
BulkCopyErrorCallback(void *arg)
{
	...
	errcontext("Bulk Copy for %s, row: %lld  (doesn't take implicit batching into consideration)",
